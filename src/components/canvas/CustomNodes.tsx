...
export const SourceNode = memo(({ data }: NodeProps) => {
  ...
  return (
    <div
      className={`w-32 h-20 rounded-lg shadow-lg border-2 border-white dark:border-gray-700 ${getNodeTypeColor(nodeData.type)} flex items-center justify-center relative`}
    >
      <Handle
        type="source"
      ...
      {/* Status Indicator */}
      <div
        className={`absolute -top-2 -right-2 w-4 h-4 rounded-full ${getNodeStatusColor(nodeData.status)} border-2 border-white dark:border-gray-700`}
      />
    </div>
  )
})
